...
with Progress(
    ...
    for caminho, nome in arquivos:
        ...
        if resultado and "acoes" in resultado:
            ...
            progress.console.print(f"  [green]✓[/] {nome} → [bold]{len(acoes)}[/] ação(ões)")
            for acao in acoes:
...
if dataset_final:
    tabela = Table(title=f"Dataset de Automação ({len(dataset_final)} ações)", show_lines=True)
    ...
    console.print(
        "[bold red]✗ Nenhuma ação gerada.[/] Verifique os arquivos e a conexão com o Ollama."
    )
