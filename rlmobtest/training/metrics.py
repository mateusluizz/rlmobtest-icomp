...
def setup_logging(run_id: str, logs_path: Path):
    ...
    All loggers under the ``rlmobtest`` package hierarchy (e.g.
    rlmobtest.phases.phase_0b_crawl, rlmobtest.metrics.*) write to the same
    run log file so that Phase 0b/0c/2 messages are visible alongside the
    training loop messages.
    """
    run_log_path = logs_path / f"run_{run_id}.log"

    # File handler shared by all loggers in this run
    ...
    # Named run logger (used directly in runner.py)
    run_logger = logging.getLogger(f"rlmobtest_{run_id}")
    run_logger.setLevel(logging.DEBUG)
    run_logger.propagate = False
    run_logger.addHandler(file_handler)

    # Package-level logger so rlmobtest.phases.*, rlmobtest.metrics.* etc.
    # also write to the run log.  Attach only once per file path to avoid
    # duplicate lines when multiple runs happen in the same process.
    pkg_logger = logging.getLogger("rlmobtest")
    pkg_logger.setLevel(logging.DEBUG)
    already_attached = any(
        isinstance(h, logging.FileHandler) and h.baseFilename == str(run_log_path.resolve())
    ...
    return run_logger, run_log_path
